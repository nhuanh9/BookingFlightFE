            ...
            <StyledContent>
                <Formik
                    ...
                    onSubmit={(values, {setSubmitting}) => {
                        console.log("trying to submit:", values);
                        if (value.searchParams.get('way') == 'one') {
                            alert('Thank you!')
                            navigate("/");
                            formSubmition(values, departureSeats, setSubmitting);
                        } else {
                            alert('Thank you! Choose return ticket!')
                            dispatch(fetchFlights({
                                query: {
                                    from: {
                                        id: value.values.to.id,
                                    },
                                    to: {
                                        id: value.values.from.id
                                    },
                                    start: value.values.start ? value.values.query.start.split(",")[0] : "",
                                ...
                                navigate({
                                    ...
                                    search: createSearchParams({
                                        from: value.searchParams.get('to'),
                                        to: value.searchParams.get('from'),
                                        start: value.searchParams.get('start'),
                                        "return":value.searchParams.get('return'),
                                        class: value.searchParams.get('class'),
                                        way: 'one'
                                    }).toString()
                                })
                            })
